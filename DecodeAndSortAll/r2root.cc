  ...
  while(!readtxt.eof())
    {
      ...
      // std::cout<<sid_tmp<<ch_tmp<<offset_tmp<<chlow_tmp<<chhigh_tmp<<std::endl;
...
#else
  extra_len += sprintf(tempname+extra_len,"%s",".root");
#endif
...
// #endif	
      ...
      for (unsigned int i = 0; i < sizelen; ++i)
	{
      ...
      // if(nevt%10000 == 0)
      // 	{
	  std::cout<< "\r" <<sortdata.size() << "\t"<<PROCESSBUFF << "\t" <<"\t"<<"nt: "<<nevt;
	  //std::cout<<"\r"<<"nt: "<<nevt;
	  std::cout << std::flush;
	// }
