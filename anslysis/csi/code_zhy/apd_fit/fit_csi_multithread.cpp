...
//int root_fit (int argc, char *argv[])
{
  ...
  if(argc == 2)
    {
      std::cout<<argv[0]<<"  [RunNnumber]"<<std::endl;
    }
  else if(argc == 3)
  {
	  std::cout<<argv[0]<<"  [RunNnumber]:"<< argv[1] << "threads:" << argv[2]<<std::endl;
  ...
  /// threads
  TString thread(argv[2]);
  int Thread = thread.Atoi();
  TString numberOfThread(argv[3]);
  int NumberOfThread = numberOfThread.Atoi();
//////////tree ����
  ...
  Long64_t nentries=tree->GetEntries();
   ...
   TFile *opf = new TFile(TString::Format("./fit_csi_%04d.root",runnumber).Data(),"RECREATE");
...
/*
    ...
    TF1 *fcsi=NULL;
  /// calculate
  int Entries = nentries / Thread ;
  int BeginEntry = Entries * NumberOfThread; 
  int EndEntry   = Entries * (NumberOfThread+1);
  std::cout << Thread << "\t" << Entries << "\t" << std::endl;
  std::cout << NumberOfThread << "\t" << BeginEntry << "\t" << EndEntry << std::endl;
  if(NumberOfThread+1 == Thread){EndEntry = nentries; } 
  
  for(Long64_t i=BeginEntry;i<EndEntry;i++){///////////////////////////////////loop start   nentries10000000nentries
  // for(Long64_t i=0;i<10000;i++){///////////////////////////////////loop start   nentries10000000nentries
  ...
  // if(cid==1 && ltra==160 && outofr==0 && pileup == 0 &&(sid == 7 && ch == 7))//////apd
  {
